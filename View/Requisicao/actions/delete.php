<?php
  ...
  $sql = "SELECT id, aluno_req, turma_req, livro_req, dataRequisicao_req, dataDevolucao_req FROM requisicao WHERE NOT status = 'devolvido'";
...
<?php 
 ...
 if (isset($_POST['request'])){
       ...
       $sql = "SELECT * FROM requisicao WHERE status = 'pendente' AND (aluno_req LIKE '%$requisicao%' OR turma_req LIKE '%$requisicao%' OR dataRequisicao_req LIKE '%$requisicao%' OR dataDevolucao_req LIKE '%$requisicao%')";
     $dados = mysqli_query($mysqli, $sql); 
...
$resultado = mysqli_query($mysqli, $sql);
 ...
 <div id="modal-sucesso" class="hide">
  ...
  <button type="submit" id="Confirmar" style="width: 42%;  height: 4vh; color: white; border: none;  background-color: #df2020;font-size:max(20px,1vw);">Confirmar</button>
  </form>
</div>
         <footer>
            <div class="quit">
